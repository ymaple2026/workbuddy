...
- **数据库**: TCB managed PostgreSQL (via CloudBase JS SDK)
- **认证**: TCB (Tencent CloudBase) Auth
...
| 字段 | 类型 | 说明 |
|------|------|------|
| id | string | 主键，与 TCB Auth user.id 对应 |
...
# TCB Auth (managed by genie-tcb-auth-integrator, auto-configured)
# VITE_CLOUDBASE_ENV_ID=xxx
# VITE_CLOUDBASE_REGION=ap-shanghai
# VITE_CLOUDBASE_PUBLISH_KEY=xxx
